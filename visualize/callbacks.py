            ...
            if df_raw is not None:
                ...
                if scope_filter == "resolved":
                    df_filtered = df_filtered[
                        (df_filtered["service_x"] != "unknown")
                        & (df_filtered["service_y"] != "unknown")
                    ]
                elif scope_filter == "unknown":
                    df_filtered = df_filtered[
                        (df_filtered["service_x"] == "unknown")
                ...
                method_column = (
                    "detection_method"
                    if "detection_method" in df_filtered.columns
                    else None
                )
                ...
                if (
                    ...
                    df_filtered = df_filtered[
                        df_filtered[method_column].str.lower()
                        == detection_method_filter
                    ]
                ...
                if (
                    comodified_filter
                    and comodified_filter != "all"
                    and "comodified" in df_filtered.columns
                ):
                    if comodified_filter == "true":
                        df_filtered = df_filtered[
                            df_filtered["comodified"].isin([True, 1])
                        ]
                    else:
                        df_filtered = df_filtered[
                            ~df_filtered["comodified"].isin([True, 1])
                ...
                if code_type_filter and code_type_filter != "all":
                    ...
                    if has_new_types:
                        if code_type_filter == "data":
                            df_filtered = df_filtered[
                                (df_filtered["file_type_x"] == "data")
                                & (df_filtered["file_type_y"] == "data")
                            ]
                        elif code_type_filter == "logic":
                            df_filtered = df_filtered[
                                (df_filtered["file_type_x"] == "logic")
                                & (df_filtered["file_type_y"] == "logic")
                            ]
                        elif code_type_filter == "test":
                            df_filtered = df_filtered[
                                (df_filtered["file_type_x"] == "test")
                                & (df_filtered["file_type_y"] == "test")
                            ]
                        elif code_type_filter == "config":
                            df_filtered = df_filtered[
                                (df_filtered["file_type_x"] == "config")
                                & (df_filtered["file_type_y"] == "config")
                            ]
                        elif code_type_filter == "mixed":
                            df_filtered = df_filtered[
                                df_filtered["file_type_x"] != df_filtered["file_type_y"]
                            ]
                    else:
                        # 古いデータ形式、または file_type カラムがない場合
                        # ファイルパスから判定する (get_file_type を使用)
                        ...
                        # apply を使う
                        df_filtered["temp_type_x"] = df_filtered["file_path_x"].apply(
                            lambda x: get_file_type(str(x))
                        )
                        df_filtered["temp_type_y"] = df_filtered["file_path_y"].apply(
                            lambda x: get_file_type(str(x))
                        )

                        if code_type_filter == "data":
                            df_filtered = df_filtered[
                                (df_filtered["temp_type_x"] == "data")
                                & (df_filtered["temp_type_y"] == "data")
                            ]
                        elif code_type_filter == "logic":
                            df_filtered = df_filtered[
                                (df_filtered["temp_type_x"] == "logic")
                                & (df_filtered["temp_type_y"] == "logic")
                            ]
                        elif code_type_filter == "test":
                            df_filtered = df_filtered[
                                (df_filtered["temp_type_x"] == "test")
                                & (df_filtered["temp_type_y"] == "test")
                            ]
                        elif code_type_filter == "config":
                            df_filtered = df_filtered[
                                (df_filtered["temp_type_x"] == "config")
                                & (df_filtered["temp_type_y"] == "config")
                            ]
                        elif code_type_filter == "mixed":
                            df_filtered = df_filtered[
                                df_filtered["temp_type_x"] != df_filtered["temp_type_y"]
                            ]

                        # 一時カラムを削除
                        df_filtered = df_filtered.drop(
                            columns=["temp_type_x", "temp_type_y"]
                        )

            from .network import create_network_graph

            return create_network_graph(
                services_data, project, language, df=df_filtered
            )
        except Exception as e:
