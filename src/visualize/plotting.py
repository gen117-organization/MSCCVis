...
def create_scatter_plot(df, file_ranges, project_name, language, static_mode=False):
    ...
    if "clone_key" not in df.columns:
        clone_key_parts = [
            ...
            df.get("start_line_x", "").astype(str)
            + "-"
            + df.get("end_line_x", "").astype(str),
            df.get("file_path_y", "").astype(str).str.split("/").str[-1],
            df.get("start_line_y", "").astype(str)
            + "-"
            + df.get("end_line_y", "").astype(str),
        ]
        df["clone_key"] = (
            clone_key_parts[0]
            + "|"
    ...
    if (intra_mask & ccfsw_mask).any():
        ...
        trace_args["hovertemplate"] = (
            "<b>サービス内クローン (Normal)</b><br>"
    ...
    if (intra_mask & tks_mask).any():
        ...
        marker_settings.update(
            dict(symbol="circle", color=tks_intra_data["heatmap_color"])
        )
        ...
        trace_args["customdata"] = list(
            zip(
                ...
                (
                    tks_intra_data["service_relative_path_y"]
                    if "service_relative_path_y" in tks_intra_data.columns
                    else tks_intra_data["service_relative_path_x"]
                ),
                tks_intra_data[method_col],
            )
        )
        trace_args["hovertemplate"] = (
            "<b>サービス内クローン (TKS)</b><br>"
            + "Clone ID: %{customdata[1]}<br>"
            + "Overlap Count: %{customdata[3]}<br>"
            + "Clone Type: %{customdata[8]}<br>"
            + "<br>"
    ...
    if (inter_mask & tks_mask).any():
        ...
        marker_settings = marker_base.copy()
        marker_settings.update(
            dict(symbol="square", color=tks_inter_data["heatmap_color"])
        )
        ...
        trace_args["customdata"] = list(
            zip(
                ...
                (
                    tks_inter_data["service_y"]
                    if "service_y" in tks_inter_data.columns
                    else tks_inter_data["service_x"]
                ),
                ...
                (
                    tks_inter_data["service_relative_path_y"]
                    if "service_relative_path_y" in tks_inter_data.columns
                    else tks_inter_data["service_relative_path_x"]
                ),
                tks_inter_data[method_col],
            )
        )
        trace_args["hovertemplate"] = (
            "<b>サービス間クローン (TKS)</b><br>"
            + "Clone ID: %{customdata[1]}<br>"
            + "Overlap Count: %{customdata[4]}<br>"
            + "Clone Type: %{customdata[9]}<br>"
            + "<br>"
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(
                ...
                cmax=max_overlap,
                colorbar=dict(
                    title=dict(text="クローン集中度<br>(重複数)", side="right"), x=1.02
                ),
            ),
            showlegend=False,
            hoverinfo="skip",
    ...
    fig.update_layout(
        ...
        height=1100,
        showlegend=False,
        xaxis=dict(title=dict(font=dict(color="gray")), tickfont=dict(color="gray")),
        yaxis=dict(title=dict(font=dict(color="gray")), tickfont=dict(color="gray")),
        # プロジェクト変更時の強制更新用
